...
argssp = argsubparsers.add_parser("hash-object", help="Compute object ID and optionally creates a blob from a file")
argsp.add_argument(
    "-t",
    metavar="type",
    dest="type",
...
argsp.add_argument(
    ...
    metavar="message",
    dest="message",
    help="Message to associate with commit",
)
...
def tree_serialize(obj):
    ...
    for i in obj.items:
        ...
        ret += b"\x00"
        # Convert the hexadecimal SHA-1 string to an integer.
        sha = int(i.sha, 16)
        # Append the 20-byte binary representation of the SHA-1 hash to the return value.
        # SHA-1 hashes are 160 bits, which is 20 bytes. 'big' byteorder means the most significant byte comes first.
...
def object_hash(fd, fmt, repo=None):
    ...
    match fmt:
        ...
        case b"blob":
            obj = GitBlob(data)
        case _:
            raise Exception(f"Unkown type {fmt}!")
...
def add(repo, paths, delete=True, skip_missing=False):
    ...
    for path in paths:
        ...
        clean_paths.add((relpath, abspath))
    ...
    for path in sorted_paths:
        ...
        for entry in contents[path]:
            if isinstance(entry, GitIndexEntry):
                leaf_mode = f"{entry.mode_type:02o}{entry.mode_type:04o}".encode("ascii")
